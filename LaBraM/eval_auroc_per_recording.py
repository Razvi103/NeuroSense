...
Requires test.h5 to contain a ``recording_ids`` dataset (produced by
the updated make_TUSZ.py).

Usage:
    # Run inference from a checkpoint
    python eval_auroc_per_recording.py \
        --checkpoint /path/to/checkpoint-best.pth \
        --data_path /path/to/TUSZ_19ch \
        --dataset TUSZ

    # Use cached probabilities (from extract_probs.py or benchmark_runs.py)
    python eval_auroc_per_recording.py \
        --cached_probs /path/to/test_probs.npz \
        --data_path /path/to/TUSZ_19ch

    # Compare multiple checkpoints
    python eval_auroc_per_recording.py \
        --cached_probs model_a.npz model_b.npz \
        --labels "Baseline" "Single GRL" \
        --data_path /path/to/TUSZ_19ch \
        --plot auroc_violin.pdf
"""
...
def compute_per_recording_auroc(y_prob, y_true, recording_ids):
    ...
    Returns (aurocs, skipped_count, recording_labels) where
    recording_labels maps each AUROC to its recording ID.
    """
