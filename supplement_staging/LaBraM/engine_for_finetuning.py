# Adapted from LaBraM (Jiang et al., 2024) -- https://github.com/935963004/LaBraM
# Original license: MIT
# Changes in this file: added get_grl_lambda, train_adversarial_batch, and
#   train_one_epoch_adversarial for adversarial training with GRL scheduling;
#   fixed deprecated torch.cuda.amp.autocast calls; fixed batch indexing.
#
# --------------------------------------------------------
...
def train_one_epoch_adversarial(
    ...
    grl_lambda = get_grl_lambda(epoch, total_epochs, adv_gamma)
    model.module.set_grl_lambda(grl_lambda) if hasattr(model, 'module') else model.set_grl_lambda(grl_lambda)
    ...
    for data_iter_step, (samples, targets, patient_ids) in enumerate(
        ...
        patient_ids = patient_ids.long().to(device, non_blocking=True)
        ...
        if is_binary:
            class_acc = utils.get_metrics(
                ...
                targets.detach().cpu().numpy(), ["accuracy"], is_binary,
            )["accuracy"]
        else:
            class_acc = (output.max(-1)[-1] == targets.squeeze()).float().mean()
