"""
Full LOPOCV re-evaluation: produces a results table from existing checkpoints,
covering raw and rule-based post-processing with SzCORE event-based metrics.
"""
...
def main():
    ...
    print(f"  Post-processing: smooth={args.smooth}, t_high={args.t_high}, "
    ...
    for fold_idx, test_pid, fold_dir, ckpt_path in folds:
        ...
        print(f"    rule_based: F1={ht_metrics['f1']:.4f}  "
              f"ScEvF1={ht_metrics['szcore_evt_f1']:.4f}  "
              f"ScEvPrc={ht_metrics['szcore_evt_precision']:.4f}")

    elapsed = time.time() - start
    print(f"\nEvaluation completed in {datetime.timedelta(seconds=int(elapsed))}")
    ...
    for strategy in strategies:
        ...
        print(f"\n  Strategy: {strategy.upper()}  ({len(fold_results)} folds)")
